...
namespace EgovTestBot.Dialogs
{
    ...
    public class QueryRecognition
    {
        ...
        { 
            ...
            AllIntentDetection aid = new AllIntentDetection();

           var intentsLIst =  await aid.GetLuisResult(result.Query);
           
            for (int i = 0; i < intentsLIst.Intents.Count; i++)
            {
                string roundedScore = intentsLIst.Intents[i].Score != null ? (Math.Round(intentsLIst.Intents[i].Score.Value, 2).ToString()) : "0";
                if (Double.Parse(roundedScore) > 0.25)
                {
                        await context.PostAsync($"**Query**: {intentsLIst.Query}, **Intent**: {intentsLIst.Intents[i].Intent}, **Score**: {roundedScore}. **Entities**: {intentsLIst.Entities[i].Type}");
                        queryresult += getInfoFromTable.By(intentsLIst.Intents[i].Intent, intentsLIst.Entities[i].Type) + "\n\n\n\n";
